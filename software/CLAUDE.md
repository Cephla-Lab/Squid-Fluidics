...
- The run-time estimate is priced by replay (`fluidics/pricing.py`): `build_worker` runs the sequences against a simulated twin of the config and bills the recorded chains plus an explicit tariff (valve moves, `set_temperature`), so dumps, tubing turnover and quantized speed codes price themselves; a replay that fails logs and falls back rather than blocking the run. The display's elapsed clock is `RunControl.running_seconds()` -- wall time minus held spans, restarted per job by the session -- and the GUI's tick reads and paints it.
