...
def build_write_multiple_registers_frame(
    slave_id: int, address: int, values: list[int]
) -> bytes:
    count = len(values)
    byte_count = count * 2
...
class ModbusRTUClient:
    ...
    def read_register_32bit(
        self, slave_id: int, address: int, signed: bool = False
    ) -> int:
        self._require_connected()
    ...
    def write_register_32bit(
        self, slave_id: int, address: int, value: int, signed: bool = False
    ):
        self._require_connected()
