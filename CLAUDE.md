...
- **`fluidics/sensor_series.py`** — `SensorSeries`, the one sample buffer: producers append, a GUI reads a window on its own clock; the plots hold their series in these. Recording is not here — each plot writes its own CSV, the flow one carrying the draw-protection fault column
