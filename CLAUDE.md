...
- **`fluidics/pricing.py`** — `price_run(config, sequences)`: the run-time estimate, priced by replaying the sequences against a simulated twin of the config and billing the chains the operations actually queue; never touches hardware, falls back rather than blocking a run
